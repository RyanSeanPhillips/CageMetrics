@echo off
REM ====================================================================
REM CageMetrics Build Script - Creates Windows Executable
REM ====================================================================
REM This script automates the process of building a Windows executable
REM from the CageMetrics behavioral analysis application using PyInstaller.
REM ====================================================================

echo.
echo ====================================================================
echo Building CageMetrics Windows Executable
echo ====================================================================
echo.
...
pyinstaller --clean cagemetrics.spec

REM Check if build was successful (looking for versioned output)
if exist "dist\CageMetrics_v%VERSION%\CageMetrics_v%VERSION%.exe" (
    echo.
    echo ====================================================================
    echo BUILD SUCCESSFUL!
    echo ====================================================================
    echo.
    echo Executable created: dist\CageMetrics_v%VERSION%\CageMetrics_v%VERSION%.exe
    echo Build directory: dist\CageMetrics_v%VERSION%\
    echo File size:
    for %%A in ("dist\CageMetrics_v%VERSION%\CageMetrics_v%VERSION%.exe") do echo %%~zA bytes
    echo.
    ...
    python -c "import shutil; shutil.make_archive('dist/CageMetrics_v%VERSION%_Windows', 'zip', 'dist', 'CageMetrics_v%VERSION%')"
    if exist "dist\CageMetrics_v%VERSION%_Windows.zip" (
        echo.
        echo ZIP FILE CREATED!
        for %%A in ("dist\CageMetrics_v%VERSION%_Windows.zip") do echo Zip size: %%~zA bytes
        echo Location: dist\CageMetrics_v%VERSION%_Windows.zip
        echo.
        echo Ready to upload to GitHub releases!
    ) else (
        echo.
        echo Warning: Failed to create zip file
        echo You can manually zip the folder: dist\CageMetrics_v%VERSION%
    )
    echo.
) else (
    echo.
