# -*- mode: python ; coding: utf-8 -*-
"""
PyInstaller spec file for CageMetrics - Behavioral Analysis Application
This file configures how PyInstaller packages the application into a Windows executable.
"""

import os
from PyInstaller.utils.hooks import collect_data_files, collect_submodules

# Get the directory containing this spec file
spec_root = os.path.dirname(os.path.abspath(SPEC))

# Import version for naming the executable
import sys
sys.path.insert(0, spec_root)
from version_info import VERSION_STRING

block_cipher = None

# Collect additional data files
added_files = [
    # Core modules
    (os.path.join(spec_root, 'core'), 'core'),
    # Dialogs
    (os.path.join(spec_root, 'dialogs'), 'dialogs'),
]

# Add images folder if it exists
if os.path.exists(os.path.join(spec_root, 'images')):
    added_files.append((os.path.join(spec_root, 'images'), 'images'))

# Hidden imports - modules that PyInstaller might miss
hidden_imports = [
    # PyQt6 modules
    'PyQt6.QtCore',
    'PyQt6.QtGui',
    'PyQt6.QtWidgets',

    # Scientific computing
    'numpy',
    'pandas',
    'scipy',
    'scipy.signal',
    'scipy.ndimage',
    'scipy.stats',

    # Matplotlib backends
    'matplotlib',
    'matplotlib.backends.backend_qtagg',
    'matplotlib.backends.backend_qt5agg',
    'matplotlib.figure',
    'matplotlib.pyplot',

    # Excel support
    'openpyxl',
    'python_calamine',

    # Core application modules
    'core.data_loader',
    'core.analysis',
    'core.figure_generator',
    'core.data_exporter',
    'core.consolidator',
    'core.config',
    'core.telemetry',
    'core.update_checker',

    # Dialogs
    'dialogs.help_dialog',
    'dialogs.first_launch_dialog',
    'dialogs.metadata_editor_dialog',
    'dialogs.npz_metadata_editor',

    # Telemetry dependencies
    'requests',
    'urllib.request',
    'urllib.error',

    # Standard library modules that might be missed
    'csv',
    'json',
    'pathlib',
    're',
    'sys',
    'os',

    # Fix for setuptools issues
    'setuptools',

    # Add missing modules that SciPy/NumPy need
    'unittest',
    'unittest.mock',
    'numpy.testing',
]

a = Analysis(
    ['main.py'],
    pathex=[spec_root],
    binaries=[],
    datas=added_files,
    hiddenimports=hidden_imports,
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
    excludes=[
        # Exclude unused modules to reduce size
        'tkinter',
        'test',
        # Exclude PySide6 to avoid Qt binding conflicts
        'PySide6',
        'PySide6.QtCore',
        'PySide6.QtGui',
        'PySide6.QtWidgets',
        # Exclude problematic modules
        'pkg_resources',
        # Exclude numba and dependencies (not used)
        'numba',
        'llvmlite',
    ],
    win_no_prefer_redirects=False,
    win_private_assemblies=False,
    cipher=block_cipher,
    noarchive=False,
)

pyz = PYZ(a.pure, a.zipped_data, cipher=block_cipher)

# Directory distribution (FAST startup)
exe = EXE(
    pyz,
    a.scripts,
    [],
    exclude_binaries=True,
    name=f'CageMetrics_v{VERSION_STRING}',
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=True,
    console=False,
    disable_windowed_traceback=False,
    target_arch=None,
    codesign_identity=None,
    entitlements_file=None,
    # icon=os.path.join(spec_root, 'images', 'cagemetrics.ico'),  # Uncomment when icon exists
)

coll = COLLECT(
    exe,
    a.binaries,
    a.zipfiles,
    a.datas,
    strip=False,
    upx=True,
    upx_exclude=[],
    name=f'CageMetrics_v{VERSION_STRING}',
)
